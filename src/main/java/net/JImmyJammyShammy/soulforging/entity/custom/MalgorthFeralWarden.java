...
public class MalgorthFeralWarden extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);
    public MalgorthFeralWarden(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(6, new FollowMobGoal(this, 0.5f, 4f, 25));
    ...
    private PlayState predicate(AnimationState<GeoAnimatable> geoAnimatableAnimationState) {
        if(geoAnimatableAnimationState.isMoving()){
            geoAnimatableAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.MalgorthFeralWarden.run", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }

        geoAnimatableAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.MalgorthFeralWarden.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
