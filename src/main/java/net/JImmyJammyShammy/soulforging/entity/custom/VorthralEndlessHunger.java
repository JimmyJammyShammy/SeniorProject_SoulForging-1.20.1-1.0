...
public class VorthralEndlessHunger extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);
    public VorthralEndlessHunger(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    private PlayState predicate(AnimationState<GeoAnimatable> geoAnimatableAnimationState) {
        if(geoAnimatableAnimationState.isMoving()){
            geoAnimatableAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.vorthral_endless_hungerl.run", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }

        geoAnimatableAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.vorthral_endless_hungerl.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
}
